...
public class ContaBancaria {
    ...
    private ArrayList<RegistroTransacao> transacoes;//Declarando um array de transacao
	...
	public ContaBancaria(int numeroConta, double saldo) {
		...
		this.saldo = saldo;
		this.status = true;
        transacoes = new ArrayList<>();//Sempre que for instanciada uma conta, será instanciada um array de trnasações
	...
	public void depositar(double valor) {
		if(valor<=0) {
			...
			saldo+=valor;
            transacoes.add(new RegistroTransacao(LocalDateTime.now(), valor,TipoTransacao.DEPOSITO));//criando um objeto do tipo RegistroTransacao e adicionando o valor do enumerator. Foi chamado um método now da classe localDateTime para adicionar o horário que o sistema operacional guarda.
		}
	}
	...
	public void sacar(double valor) {
		if(!status) {
			...
			saldo-=valor;
            transacoes.add(new RegistroTransacao(LocalDateTime.now(), valor, TipoTransacao.SAQUE));//criando um ojeto do tipo RegistroTransacao e adicionando o valor do enumerator. Foi chamado um método now da classe LocalDateTime para adicionar o horário que fica guardado no SO
		}
	...
	public void realizarTransferencia(double quantia, ContaBancaria destino) {
		 if(!status) {
            ...
            transacoes.add(new RegistroTransacoes(LocalDateTime.now(),quantia,TipoTransacao.TRANSFERENCIA_DEBITADA ));//Adicionando o Registro da transação no objeto conta origem
            destino.transacoes.add(new RegistroTransacoes(LocalDateTime.now(),quantia,TipoTransacao.TRANSFERENCIA_CREDITADA ));//Adicionando o Registro da transação no objeto conta destino
    ...
    public void imprimirExtrato(int mes, int ano) {
        ...
        for(RegistroTransacao extra: transacoes){//foreach que encontra a primeira ocorrencia do objeto extra no array transacoes
            if(extra.getDataTransacao().getMonth().getValue()== mes && extra.getDataTransacao().getYear()==ano){//verifica se há um extrato no mês e ano declarados no método
                saldoExtrato+= extra.getValor();//adiciona o valor do saldo na variável saldo extrato, pois o objeto irá ser descartado pelo Garbage collector ao terminar o foreach
                System.out.println(extra);//impressãodo objeto extra com a transação do referido mês e ano
            }
            System.out.println("Immpressão do saldo referente ao extrato: "+saldoExtrato);
        }
    }
